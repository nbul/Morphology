...
usedefault = questdlg(strcat('Do you want to re-select ROI'),'Settings','Yes','No','No');
...
for loop = 1:numel(files)
    ...
    if choice == 0
        image1=figure;
        imshow(ImRes);
        ROI = drawcircle('Color','r','FaceAlpha',0.4);
        image_filename = append(regexp(files(loop).name,'\d*','Match'),'_ROI.tif');
        print(image1, '-dtiff', '-r150', char(image_filename));
        x = ROI.Center(1);
        y = ROI.Center(2);
        Radius = ROI.Radius;
        roi_all(loop,:) = [str2double(regexp(files(loop).name,'\d*','Match')); ROI.Center(1);ROI.Center(2); ROI.Radius];
        close all
    else
        data = table2array(readtable('ROI.xlsx'));
        x = data(data(:,1) == str2double(cell2mat(regexp(files(loop).name,'\d*','Match'))),2);
        y = data(data(:,1) == str2double(cell2mat(regexp(files(loop).name,'\d*','Match'))),3);
        Radius = data(data(:,1) == str2double(cell2mat(regexp(files(loop).name,'\d*','Match'))),4);        
    end
